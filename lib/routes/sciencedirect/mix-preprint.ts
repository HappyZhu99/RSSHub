...
export const route: Route = {
    path: '/journal/:id/mix',
    categories: ['journal'],
    example: '/sciencedirect/journal/journal-of-financial-economics/mix',
    ...
    name: 'Current Issue and Articles in Press Mix',
...
async function handler(ctx) {
    ...
    const previewUrl = `${currentUrl}/articles-in-press`;
    ...
    const previewResponse = await ofetch(previewUrl, {
        headers: {
            'User-Agent': config.trueUA,
        },
    });
    const $pre = load(previewResponse);
    const preData = JSON.parse(JSON.parse($pre('script[type="application/json"]').text()));
    const preIssue = preData.articles.aip.data;

    const mixIssue = preIssue.issueBody.concat(currentIssue.issueBody.issueSec[0].includeItem);
    const list = mixIssue.map((item) => ({
        title: item.title,
